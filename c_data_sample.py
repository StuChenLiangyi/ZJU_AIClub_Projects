...
#         y_pred =np.rint( bst.predict(valid_X,num_iteration=bst.best_iteration))
#         # y_pred = clf.predict(x_train.drop(['id'], axis=1))
#         print("lightgbm Accuracy : %.4g" % accuracy_score(
#             list(valid_y), y_pred))
...
#         print(pd.DataFrame(eval_preds)[0].describe())
...
#             'is_same_prov','age_bin','lmt_bin','setupHour_bin','dist_bin']))
    ...
    for i, (train_index, test_index) in enumerate(fold):
        print("\n[{}] Fold {} of {}".format(
            datetime.datetime.now().strftime("%Y-%m-%d %H:%M"), i+1, NFOLD))
        train_X, valid_X = df[predictors].loc[train_index], df[predictors].loc[test_index]
        train_y, valid_y = df[label].values[train_index], df[label].values[test_index]

        eval_results = {}
        clf = ctb.CatBoostClassifier(iterations=500, depth=6, learning_rate=0.2, loss_function='Logloss',
                                     logging_level='Verbose', custom_metric='AUC',
                                     eval_metric='AUC', scale_pos_weight=0.15)
        ...
        eval_score += clf.best_score_['validation']['AUC']

        eval_preds[test_index] += y_predprob
#         print(pd.DataFrame(eval_preds)[0].describe())
...
#         selected_id = list(df_av_train[df_av_train.preds <= rate].id)
    selected_id = list(df_av_train.head(neighbor_nums).id)  # 服了，直接选最接近的8w条
    # selected_id = list(df_av_train.loc[5000: (5000 + neighbor_nums)].id)
    selected_id += list(b_feature_engineering.label_1_df.id)
    train_selected = b_feature_engineering.train[b_feature_engineering.train.id.isin(
        selected_id)] 
    print("已筛选出和测试集近邻点", len(selected_id))
...
def plot_resampling(ax, X, y, title):
    c0 = ax.scatter(X[y == 0, 0], X[y == 0, 1], label="Class #0", alpha=0.1)
    c1 = ax.scatter(X[y == 1, 0], X[y == 1, 1], label="Class #1", alpha=0.1)
    ax.set_title(title)
